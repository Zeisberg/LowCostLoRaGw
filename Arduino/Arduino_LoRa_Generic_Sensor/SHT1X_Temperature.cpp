...
SHT1X_Temperature::SHT1X_Temperature(char* nomenclature, bool is_analog, bool is_connected, bool is_low_power, uint8_t pin_read, uint8_t pin_power):Sensor(nomenclature, is_analog, is_connected, is_low_power, pin_read, pin_power){
  if (get_is_connected()){
...
void SHT1X_Temperature::update_data()
{
  if (get_is_connected()) {
    ...
    float tempC = sht->readTemperatureC();
	set_data((double)tempC);
   
 
    Serial.print("SHT10 Temperatur: ");
    Serial.print(tempC);
    Serial.println(" C");
    ...
    if (get_is_low_power())		
  	...
  	if (has_fake_data()) 	{
      Serial.println("Set random data for SHT10 Temperature");
...
double SHT1X_Temperature::get_value()
{
  update_data();
  return get_data();
}
